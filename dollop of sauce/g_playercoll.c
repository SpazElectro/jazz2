...
void SolidPlayers(int nump,int *newxpos,int *newypos)
{
	int t,dx,dy,speed,hurt;
	Word8 data[64];
	TactPACKETS *action;

	
	if (player[nump].flicker || player[nump].spring) 
	{
		if (player[nump].lift>=PLAYSLOT)
			player[nump].lift=0;
		if (player[nump].platform>=PLAYSLOT)
		{
			player[nump].platform=0;
			player[nump].platformtype=0;
		}
		return;//not solid
	}
	...
	if ((t!=nump) && (player[t].Active))
	{
		if (player[t].flicker)
			goto ClearInfo;

		if ((MultiPlayerType==gameCOOPERATIVE) &&		//player is dead, get out
			(player[t].energy<=0) && (player[t].lives<0))
			goto ClearInfo;

		if ((MultiPlayerType==gameCAPTURE) &&			//same team, get out
			(player[t].Team==player[nump].Team))
			goto ClearInfo;	//don't hit players of same team

		if (player[t].energy<=0)		//always...???????????????????????????
			goto ClearInfo;



		hurt=0;
		dx=player[t].xpos-*newxpos;
		dy=player[t].ypos-*newypos;
		
		if ((dx>-16*65536) && (dx<16*65536) &&
			(dy>-24*65536) && (dy<24*65536))
		{
...
//				hurt=((player[nump].downattack<DOWNATTACKLEN) && (player[nump].yspeed>6*65536));
//downattack checked below (based on relative Ypositions)
				hurt=0;
				...
				{
					if ((player[nump].character==mSPAZ) && (player[nump].specialmove>10))
						hurt=-1;
					else
					if ((player[nump].character==mJAZZ) && (player[nump].specialmove>16))
						hurt=-1;
				}

				if (hurt)
				{
					HitPlayer(t,PLAYSLOT+nump);
				}
			}	//special moves!
			...
			{
				if (dx<0)
					*newxpos=*newxpos+(16*65536+dx);
				else
				if (dx>0)
					*newxpos=*newxpos-(16*65536-dx);

				speed=ABS(player[nump].xspeed)+ABS(player[t].xspeed);

				player[t].yspeed=-speed;
				player[nump].yspeed=-speed;
				if (dx<0)
				{
					player[t].xspeed=-speed;
					player[nump].xspeed=speed;
				} else
				{
					player[t].xspeed=speed;
					player[nump].xspeed=-speed;
				...
				player[t].idletime=player[nump].idletime=0;
			} else
			if (dy<0) //this player below
			{
				*newypos=*newypos+(24*65536+dy);
				if ((player[t].platform!=PLAYSLOT+t) && (player[t].yspeed==0))
				{
					player[t].platform=PLAYSLOT+t;
					player[t].platformtype=3;

					if (MultiPlayerType==gameCOOPERATIVE)
						player[nump].lift=PLAYSLOT+t;
				}
			}
			else
			if (dy>0)
			{
				...
				*newypos=player[nump].ypos; //dont move

				if (player[nump].platform!=PLAYSLOT+nump)
				{
					player[nump].platform=PLAYSLOT+nump;
					player[nump].platformtype=3;

					if ((MultiPlayerType==gameCOOPERATIVE) && (player[t].yspeed==0))
						player[t].lift=PLAYSLOT+nump;
				}

				player[nump].movexspeed=0;
				player[nump].moveyspeed=0;
				
				player[nump].idletime=player[t].idletime=0;

				if (player[nump].downattack<DOWNATTACKLEN) 
				{
					if (player[nump].yspeed>6*65536)
					{
						if ((MultiPlayerType==gameBATTLE) || 
							(MultiPlayerType==gameCAPTURE) || 
							(MultiPlayerType==gameTREASURE))
						...
						player[nump].yspeed=-(player[nump].yspeed/2)-8*65536;
						player[nump].yacc=0;
					} else
						player[nump].yspeed=0;

					player[nump].downattack=DOWNATTACKLEN;
				} else
					player[nump].yspeed=0;

				player[nump].fixanim=player[t].fixanim=0;
			} //dy>0 (this player on top)				
		}
		else
			goto ClearInfo;	//not hittin


//HURTIN'
		if (hurt && (MultiPlayerConnection>gameLOCAL))
		{
			action=(TactPACKETS *)&data[0];

			action->GENERIC.PacketID=actPLAYERSTOMP;
			action->PLAYERSTOMP.nump=t;
			if (player[t].energy>0)
				action->PLAYERSTOMP.energy=player[t].energy;
			else
				action->PLAYERSTOMP.energy=0;
			action->PLAYERSTOMP.creator=nump;
		...
		continue;
ClearInfo:
		{	//remove platform info and such

			if ((dx>-16*65536) && (dx<16*65536) &&
				(dy>-28*65536) && (dy<28*65536))
			{
				//execute this comment. haha! die fucking C compilers
...
void PredictPlayer(int nump)
{
	...
	if (
		(gamecounter>play->LastReceivedGameCounter+70) || 
		(play->energy<=0)
		)
	{

		play->xpos=play->oldxpos;	//set new position directly
		play->ypos=play->oldypos;
		play->ErrorStep=0;
	...
	mxspeed=play->xspeed;
	if (mxspeed>8*65536) mxspeed=8*65536;		//only 8? woa
	else
	if (mxspeed<-8*65536) mxspeed=-8*65536;

	newx=play->xpos+mxspeed;
	newy=play->ypos+myspeed;
	...
	if ((dx<-2048) || (dx>2048) || (dy<-2048) || (dy>2048))	//optimization (2048*8=16384)
	{
		px=(newx-12*65536)/65536;
		py=(newy-4*65546)/65536;
	...
	//start checking 24x24 block
		...
		if (result)
		{
			//find last position that kinda works ... apx
			newx=oldx;
			newy=oldy;
...
//only for trying out stuff, you'll never get there if this flag is 'wrong'
	...
	if ((play->yspeed<0) && !play->goup)	//dont shoot through ceiling
	{
		play->yspeed=0;
		if (play->ErrorY<0)
			play->ErrorY=0;
...
//collision with springs
	{
		...
		if (gameobj[numo].ProcPtr==&spring)
		{
			dy=play->ypos-gameobj[numo].ypos;

			if ((dy>-32*65536) && (dy<32*65536))
			{
				testx=(play->xpos-gameobj[numo].xpos)/65536;		//changed this: only sets hit flag when the spring WORKS
				testx2=ABS(play->xspeed/65536)+6;
				if (testx2<16) testx2=16;

				if (ABS(testx)<testx2)
				{
					obj=(Omonster *)(&gameobj[numo]);

					if ((obj->freeze==0) && (obj->var3>=obj->var2))
					{
						numhits=PixelCollision((Tgameobj *)play,&gameobj[numo]);
						if (numhits>7)	//???
						{
							obj->special=nump+PLAYSLOT;
							obj->state=sSPRING;
						}
					}
				}	//dy
...
void CheckDestructScenery(int nump,int newxpos,int newypos)
{
	int			n;
	int			downattack;
	int			numhits;
	...
	Omonster	*obj;
	Tplayer		*play;
	int			storexpos,storeypos;
	TactPACKETS *action;
	Word8	data[128];

	play=&player[nump];
	storexpos=play->xpos;		//back up orignal player strucs
	storeypos=play->ypos;
	play->xpos=newxpos;			//need this for pixelcollision, gets coordinates
	play->ypos=newypos;			//out of players structure
	...
	if ((gameobj[n].ProcPtr==&cDESTRUCTSCENERY) && 
		(gameobj[n].state!=sDONE))
	{
		numhits=PixelCollision((Tgameobj *)play,&gameobj[n]);
		if (numhits>0)
		{
			obj=(Omonster *)(&gameobj[n]);

			if (obj->curanim==(int)AnimBase[mDESTSCEN]+mDESTSCEN_48)
			{	//speed destruct scenery!

				if ((ABS(play->xspeed)>8*65536) ||
					((ABS(play->yspeed)>8*65536) && (ABS(newxpos-obj->xpos)<16*65536)) ||
					downattack)
				{
					DoPoints(nump+PLAYSLOT,n);
					obj->energy=-1;
					obj->state=sKILL;
					obj->ProcPtr(n); //let this thing delete the tile b4 player collision!

					if (MultiPlayerConnection>gameLOCAL)
					{

						action=(TactPACKETS *)&data[0];
						action->GENERIC.PacketID=actOBJECTSTOMP;
						action->OBJECTSTOMP.UIDLow=UID[n];
						action->OBJECTSTOMP.UIDHigh=UID[n]>>8;
						if (!LocalComputerIsServer)
							Client_SendActions(action);
						else
							Server_SendActions(action,0);	//send to all!
			...
			if ((obj->load==aSTOMPSCENERY) && downattack)
			{
				dx=(newxpos-obj->xpos)/65536;
				dy=(newypos-obj->ypos)/65536;
				absdx=ABS(dx);
				absdy=ABS(dy);

				if (((downattack==-1) && (absdy<20)) || //SPAZ KICK MOVE
					((downattack==-2) && (absdx<28) && (dy>0)) || //JAZZ UPSTOMP MOVE 
					((downattack==1) && (dy<0) && (absdx<25)))						//normal buttstomp
				{
					DoPoints(nump+PLAYSLOT,n);
					obj->energy=-1;
					obj->state=sKILL;
