...
void cUNFREEZE(int numo,int type)
{
	int cobj;
	Tgameobj *obj;
	Omonster *obj2;
	...
	if (numo>=PLAYSLOT)
		obj=(Tgameobj *)(&player[numo-PLAYSLOT]);
	else
	{
		ASSERT(numo<LEVELSLOT);
		obj=&gameobj[numo];
	}
	...
	if (type==0)
		numparts=16+RandFac(15);
	else
	if (type==1)
		numparts=32+RandFac(15);
	else
		numparts=6+RandFac(7);

	do {
		cobj=AddObject(obj->xpos+Rand2Fac(32767),obj->ypos-RandFac(32767),aSHARD,0);
		if (cobj>0)
		{
			obj2=(Omonster *)(&gameobj[cobj]);
			obj2->curanim=AnimBase[mPICKUPS]+mPICKUPS_ISHARD1+(Word16)RandFac(3);
		} else {
			break;
		}
	} while (--numparts);

	PlaySample(obj->xpos,obj->ypos,sCOMMON_ICECRUSH,0,0);
...
//CHECKING FOR BOUNCING OF WALLS OR RAVINES
	...
	if ((hit2!=0) || (hit1==0) || (tileattr==areaSTOPENEMY))	//bounce of wall
	{
		if (obj->xspeed>=0)
			px=-frames[obj->curframe].tx2-frames[obj->curframe].dx;
		else
		...
		if (hit2!=0)	//dont do the direction swap
		{
			obj->state=sWAIT;
		}
		else
		{
			if (tileattr!=areaSTOPENEMY)
			{
	...
	if (obj->xspeed<0)
		...
		obj->direction=1;	//check other objects, if not/.... TO DO
...
void cLIGHTEXPLODE(int num)
{
	...
	else
	{
		...
		{
			obj->light--;
			if (obj->light<2) obj->lighttype=0;
		}
...
void AddHalo(int xpos,int ypos)
{
	Omonster *obj;
	int res;
	...
	if (!parLowDetail)
	{
		res=AddObject(xpos,ypos,aBULLET,0);
		if (res>0)
		{
			obj=(Omonster *)(&gameobj[res]);
			
			obj->objtype=oEXPLOSION;
			obj->lighttype=8;
			obj->light=14;
			obj->ProcPtr=&cHALOEXPLODE;
		}
	}
...
//set it again in level!
//only an event triggered from the eventmap should do this stuff!
	if (obj->creator==LEVELSLOT) //&& (obj->creator<PLAYSLOT))		
...
void cPINPADDLE(int num)	//flipper
{
	Omonster *obj;
	Tplayer *play;
//	int koppel;
	...
	case sFREEZE:
		if (obj->freeze>0)
		{
			...
			if (obj->freeze==0)
			{
				obj->state=obj->oldstate;	//set back 
			}
		}
		break;
	...
	case sACTION:
		obj->state=sSLEEP; //go back already
		//that is: we don't get an event when the
		//player is NOT touching this anymore.
		//if so: it needs to go to sleep, otherwise
	...
	if (obj->state!=sSTART)	//whaterver
	{
		int nump;
//		int dx,dy,dist;
		int spritedx,startx,endx;
		int spritedy,starty,endy;
//		int radius,radius2;
		int moment;
		int thisy;
		...
		spritedx=frames[obj->curframe].dx;
		startx=obj->xpos+frames[obj->curframe].tx2*65536;
		endx=obj->xpos+(frames[obj->curframe].tx2+spritedx)*65536;

		if (obj->load==aPINLEFTPADDLE)
			endx+=5*65536;	//better collision
		else
			startx-=5*65536;

		spritedy=frames[obj->curframe].dy;
		starty=obj->ypos+(frames[obj->curframe].ty2-16)*65536;
		endy=obj->ypos+(frames[obj->curframe].ty2+spritedy+6)*65536;
...
//53, 16height
		...
		if (player[nump].Active)
		{
			...
			if ((play->xpos>startx) && (play->xpos<endx) &&
				(play->ypos>starty) && (play->ypos<endy))
			{
				if (obj->load==aPINLEFTPADDLE)
				{
					moment=play->xpos-startx;	//from start
					thisy=starty+(moment*16)/53;

					if (play->ypos>thisy-8*65536)
					{
						play->ypos=thisy;
						play->platform=num;
						play->platformtype=4;
						...
						if (play->lastjump<3)	//press jump!
						{
							...
							moment=(play->xpos-obj->xpos);

							if (moment<=0)
							{
								play->xspeed=moment/2;
								play->yspeed=-4*65536;
							}
							else
							{
								play->xspeed=16384+moment/32;
								play->yspeed=-moment;
							
								PlaySample(obj->xpos,obj->ypos,sCOMMON_LANDPOP,0,0);
							}	//moment
						}	//if playerjumped
					}	//ypos above flipper
				}	//left flipper
				...
				{
					moment=endx-play->xpos;	//from start
					thisy=starty+(moment*16)/53;
...
void cPINBUMP(int num)
{
	...
	if (obj->state==sACTION)
	{
		...
		if (obj->load==aPINCARROTBUMP)
		{
			if (RandFac(3)==3)
				AddObject(obj->xpos,obj->ypos,aENERGYUP,0);
		}
		else
		{
			//500 extra points? what the heck?
		}

		play=&player[obj->var1];
		...
		obj->curframe=anims[obj->curanim].framelist[1];
		obj->state=sBOUNCE;

		PlaySample(obj->xpos,obj->ypos,sPINBALL_BELL,0,0);
	}
	else
	if (obj->state==sBOUNCE)
	{
		obj->counter++;
...
void cROCK(int num)
{
	...
	if (obj->state==sSTART)
	{
		obj->curanim=AnimBase[mROCK]+0;
		obj->curframe=anims[obj->curanim].framelist[0];

		obj->ypos+=12*65536;
		obj->var1=RandFac(1023); //angle
		obj->var2=4;
		obj->state=sSLEEP;
		obj->var4=GetEventParm(obj->xorg/(32*65536),obj->yorg/(32*65536),0,8);
		obj->counter=0;
	}
/*
...
*/
	...
	if (obj->state==sACTION)
	{
		if (obj->counter==0)
		{
			...
			if (player[nump].Active)
			{
				dx=(player[nump].xpos-obj->xpos)/65536;
				dy=(player[nump].ypos-obj->ypos)/65536;
				dist=dx*dx+dy*dy;

				if (dist<400*400)
				{
					dist=(400*400-dist)/(256*8);	//quake amplitude
					if (dist>40) dist=40;

					player[nump].Quake=-dist;
				}
			}
		}
		...
		if (obj->counter<40)
		{
			obj->ypos+=sintable(gamecounter*32);
			obj->xpos+=sintable(gamecounter*64);
			obj->var1+=Rand2Fac(7);//sintable(gamecounter*16)/512;
		}
		else
		{
			obj->xspeed=32768*GetEventParm(obj->xorg/(32*65536),obj->yorg/(32*65536),8,-4);
			obj->yspeed=32768*GetEventParm(obj->xorg/(32*65536),obj->yorg/(32*65536),12,-4);
	...
	if (obj->state==sFLY)
	{
		...
		goup=CheckPixel(obj->xpos,obj->ypos-33*65536);
		godown=CheckPixel(obj->xpos-4*65536,obj->ypos+36*65536)+
					CheckPixel(obj->xpos+4*65536,obj->ypos+36*65536);

		goleft=CheckPixel(obj->xpos-36*65536,obj->ypos)+
				CheckPixel(obj->xpos-19*65536,obj->ypos+19*65536)+
				CheckPixel(obj->xpos-4*65536,obj->ypos+33*65536);

		goright=CheckPixel(obj->xpos+36*65536,obj->ypos)+
				CheckPixel(obj->xpos+19*65536,obj->ypos+19*65536)+
				CheckPixel(obj->xpos+4*65536,obj->ypos+33*65536);
		...
		if (sample)
		{
			strength=0;
			if (sample&2)
				strength+=abs(obj->yspeed/8192);
			if (sample&1)
				strength+=abs(obj->xspeed/8192);
			
			//find near players and quake their screens
			for (nump=0;nump<MAXPLAYERS;nump++)
			if (player[nump].Active)
			{
				dx=(player[nump].xpos-obj->xpos)/65536;
				...
				if (dist<400*400)
				{
					dist=(400*400-dist)/(256*16);	//quake amplitude
			...
			if ((ABS(obj->xspeed)>2*65536) ||
				(ABS(obj->yspeed)>2*65536))
			{
				calc=ABS(obj->yspeed)/8192+ABS(obj->xspeed)/16384;
				PlaySample(obj->xpos,obj->ypos,sROCK_ROCK1,calc,4000);
			}
		}

		if (ABS(obj->xspeed)<32768)	//stop!
		{
			obj->xspeed=0;
			obj->yspeed=0;
			obj->xpos=(obj->xpos+32767)&0xffff0000;
			obj->ypos=(obj->ypos+32767)&0xffff0000;
			obj->state=sDONE;
		};
		...
		//Rock2Rock collision
		if (obj->var7!=(int)gamecounter)	//already collided with a rock, skip it now!
		{
			int numo;
			int dx,dy,dist;
			//int factor;
			...
			for (numo=1;numo<MAXOBJECTS;numo++)
			if ((gameobj[numo].ProcPtr==cROCK) && (numo!=num) && (gameobj[numo].state!=sACTION))
			{
				obj2=(Omonster *)(&gameobj[numo]);

				dx=(obj2->xpos-obj->xpos)/65536;
				dy=(obj2->ypos-obj->ypos)/65536;

				dist=dx*dx+dy*dy;//(dx/1024)*(dx/1024)+(dy/1024)*(dy/1024);

				if (dist<74*74)//*64*64)
				{
					relspeeds=(ABS(obj->xspeed)+ABS(obj2->xspeed))/2;//-2048)/2;

//					obj2->xpos-=(factor*dx)/1;
//					obj2->ypos-=(factor*dy)/1; 
					obj2->var7=gamecounter;

					if (dx<0)
					{
						obj2->xspeed=-relspeeds;
						obj2->xacc=-abs(obj2->xacc);
					}
					else
					{
						obj2->xspeed=relspeeds;
...
//					obj->xpos+=(factor*dx)/1;
//					obj->ypos+=(factor*dy)/1; 
					obj->var7=gamecounter;
					if (dx>0)
					{
						obj->xspeed=-relspeeds;
						obj->xacc=-abs(obj->xacc);
					}
					else
					{
						obj->xspeed=relspeeds;
		...
		rspeed=(obj->xspeed*obj->var2)/(2*65536);
		obj->var1-=rspeed;
	...
	AddRotSprite(num,obj->xpos,obj->ypos,SPRITELAYER,obj->var1/4,64,obj->curframe,0);
...

	{
		...
		if (player[nump].Active)
		{
				...
				if (dist<60*60)
				{
					if (dy>-20)
					{
						if (dx<0)
						{
							...
							play->ShiftPositionX=obj->xpos-40*65536;
							hitsome++;
						}
						else
						if (dx>0)
					...
					if (dy<-20)
					{
						testy=obj->ypos-20*65536-40*costable(dx*256/40);

						if ((play->ypos>=testy-65536) &&
							(play->ypos<testy+65536*7))
						{
							play->ypos=testy;//obj->ypos-20*65536;
							play->platform=num;
							play->platformtype=1;
			...
			if (hitsome==0)
			{
				if ((play->platform==num) || (play->pushobject==num))
				{
					play->platform=0;
					play->platformtype=0;
					play->pushobject=0;
...
void cheshire1(int num)
{
    ...
    case sFADEIN:
    	obj->curanim=AnimBase[mCAT]+mCAT_FADEIN;
	...
	case sFREEZE:
		if (obj->freeze)
		{
			obj->freeze=0;
			obj->state=sFADEOUT;
            //cheshire1(num);	//call again to go to other state
		} else 
			break;

    case sFADEOUT:
    	obj->curanim=AnimBase[mCAT]+mCAT_FADEOUT;
		if ((gamecounter&7)==0) obj->phase++;
		if (obj->phase>(int)anims[obj->curanim].numframes-1)
		{
			obj->phase=0;//anims[obj->curanim].numframes-1;
			obj->curanim=AnimBase[mCAT]+mCAT_INVISIBL;
			obj->state=sHIDE;
		};
	...
	default:
		FAIL("cheshire1(): Illegal state: obj:%i, type:%i, state:%i",num,obj->load,obj->state);
		break;
	}; //switch
...
void hatter(int num)
{
	...
	case sWALK:
		...
		if (obj->counter>70)
		{
			t=GetNearestPlayer(num,128*128);
			if (t>=0)
			{
				obj->state=sFIRE;
				obj->counter=0;
				...
				obj->var2=RandFac(3);
				obj->phase=0;
				obj->curanim=AnimBase[mHATTER]+mHATTER_SHOOT;
				if (player[t].xpos<obj->xpos) 
					obj->direction=-1;
				else
					obj->direction=1;
			};
	...
	case sFIRE:
		...
		{
			if (obj->phase==3)
				PlaySample(obj->xpos,obj->ypos,sHATTER_SPLOUT,128,0);
			else
			if ((obj->phase==12) && (obj->state==sFIRE))
			{
				PlaySample(obj->xpos,obj->ypos,sHATTER_PTOEI,128,0);
				res=AddBullet(num,aBULLET);
				if (res>=0)
				{
					bobj=(Obullets *)(&gameobj[res]);
					bobj->ProcPtr=&cBULLET1;
					bobj->xspeed=2*65536;
					bobj->xacc=8192;
					//bobj->yspeed=0;
					//bobj->yacc=0;
					...
					bobj->curanim=AnimBase[mHATTER]+mHATTER_SPIT;
					bobj->killanim=AnimBase[mAMMO]+mAMMO_XPLOS1;
					//bobj->state=sSTART;
					bobj->lighttype=2;
					bobj->lifetime=AISPEED/2;
				};

				obj->counter+=3; //skip this phase (argh)
				if (obj->var2==1)
					obj->state=sEXTRA; //show rest of anim, dont fire anymore

			}
			else
			if (obj->phase==19)
				PlaySample(obj->xpos,obj->ypos,sHATTER_SPLIN,128,0);
		};
	...
	case sKILL:
		...
		if (cobj>0)
		{
			gameobj[cobj].curanim=AnimBase[mHATTER]+mHATTER_CUP;
			((Omonster *)(&gameobj[cobj]))->var6=sHATTER_CUP;
		}
		cobj=AddObject(obj->xpos,obj->ypos,aSHARD,0);
		if (cobj>0)
		{
			gameobj[cobj].curanim=AnimBase[mHATTER]+mHATTER_HAT;
			((Omonster *)(&gameobj[cobj]))->var6=sHATTER_HAT;
		}
	...
	default:
		FAIL("hatter(): Illegal state: obj:%i, type:%i, state:%i",num,obj->load,obj->state);
		break;
	}; //switch
...
//	int t;
	...
	case sSTART:
		...
		obj->counter=0;
		obj->direction=-1;
		obj->var1=RandFac(31); //length of idle anim
		break;
	...
	case sSLEEP:
		...
		case 0:
			...
			break;
		case 1:
			obj->curanim=AnimBase[mCATERPIL]+mCATERPIL_INHALE1;
			obj->phase=(obj->counter/8);
			if (obj->phase>=(int)anims[obj->curanim].numframes)
			{
				obj->phase=anims[obj->curanim].numframes-1;

				obj->special=2;
				obj->var1=1+RandFac(3);
				obj->counter=0;
			};
			break;
		case 2:
			obj->curanim=AnimBase[mCATERPIL]+mCATERPIL_INHALOOP;
			obj->phase=(obj->counter/8);
			if (obj->phase>=(int)anims[obj->curanim].numframes)
			{
				obj->phase=anims[obj->curanim].numframes-1;
			...
			break;
		case 3:
			obj->curanim=AnimBase[mCATERPIL]+mCATERPIL_EXHALE1;
			obj->phase=(obj->counter/8);
			if (obj->phase>=(int)anims[obj->curanim].numframes)
			{
				obj->phase=anims[obj->curanim].numframes-1;
				obj->special=4;
				obj->counter=0;
				obj->var1=4+RandFac(7);	//number of smoke pufs
			};
			break;
		case 4:
			obj->curanim=AnimBase[mCATERPIL]+mCATERPIL_EXHALOOP;
			obj->phase=(obj->counter/12); //5fps
			if (obj->phase>=(int)anims[obj->curanim].numframes)
			{
				obj->phase=anims[obj->curanim].numframes-1;

				if ((obj->counter%12)==11)
				{
					obj->counter=0;
					obj->var1--;

					AddBullet(num,aCATSMOKE);
		...
		case 5:
			obj->curanim=AnimBase[mCATERPIL]+mCATERPIL_EXHALE1;
			obj->phase=anims[obj->curanim].numframes-1-(obj->counter/8);
			if (obj->phase<0)
			{
				obj->phase=0;
				obj->special=0;
	...
	case sHIT:
		obj->energy=127;
		if (obj->curanim!=(int)AnimBase[mCATERPIL]+mCATERPIL_GETSHOT)
		{
			obj->phase=0;
			obj->curanim=AnimBase[mCATERPIL]+mCATERPIL_GETSHOT;
			obj->counter=0;
		...
		{
			obj->phase++;
			PlaySample(obj->xpos,obj->ypos,sCOMMON_HIBELL,20+RandFac(31),15000+RandFac(8191));
		};

		if (obj->phase>=(int)anims[obj->curanim].numframes)
		{
			if (obj->counter<5*70)	//replay anim
			{
				obj->phase=0;
			} else
			{	//smooth transition: when anim is ended go to next state!
				obj->curanim=AnimBase[mCATERPIL]+mCATERPIL_IDLE;
	...
	default:
		FAIL("caterpillar(): Illegal state: obj:%i, type:%i, state:%i",num,obj->load,obj->state);
		break;
	}; //switch
...
void dog(int num)
{
	...
	case sWALK:
		...
		if (obj->counter>obj->var4)
		{
			...
			obj->var4=30+RandFac(31);
			if (obj->var4&1)
				PlaySample(obj->xpos,obj->ypos,sDOG_SNIF1,40,0);
		};
		break;
	...
	case sACTION:	//attack!
		obj->curanim=AnimBase[mDOG]+mDOG_ATTACK;

//if doggie is facing the way of the player, he accelerates faster then
//when NOT facing him
		...
		if (obj->direction<0)
		{
			...
			if (player[obj->var1].xpos<obj->xpos)
				obj->xspeed=-2*65536;
			else
				obj->xspeed=-2*65536;
		} else
...
//CHECKING FOR BOUNCING OF WALLS OR RAVINES
		...
		if ((hit2==1) || (hit1==0) || (tileattr==areaSTOPENEMY))	//bounce of wall
		{
			...
			obj->state=sWALK;	//walk! stop excited
			obj->xacc=0;
			obj->xspeed=obj->direction*65536;
		};
		animspeed=12-ABS(obj->xspeed/16384);
		obj->counter++;
		if (obj->counter>animspeed)
		{
			obj->counter=0;
			obj->phase++;
		};

		if ( ((obj->counter&3)==0) && (RandFac(15)==0))
		{
			switch (RandFac(3))
			{
				case 0:
					PlaySample(obj->xpos,obj->ypos,sDOG_WAF1,0,0);
					break;
				case 1:
					PlaySample(obj->xpos,obj->ypos,sDOG_WAF2,0,0);
					break;
				default:
					PlaySample(obj->xpos,obj->ypos,sDOG_WAF3,0,0);
					break;
			}; //switch
		};
	...
	default:
		FAIL("dog(): Illegal state: obj:%i, type:%i, state:%i",num,obj->load,obj->state);
		break;
	}; //switch
